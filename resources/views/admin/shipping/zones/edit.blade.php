...
<div class="admin-page-header">
    <h2>Edit Shipping Zone: {{ $zone->name }}</h2>
    <p class="admin-desc">Update regions and delivery methods for this zone.</p>
</div>
...
<form method="POST" action="{{ route('admin.shipping.zones.update', $zone) }}">
    ...
    <section class="postbox">
        ...
        <div class="inside">
            <div class="admin-form-row">
                ...
                <input type="text" id="zone_name" name="name" value="{{ old('name', $zone->name) }}" required>
            </div>
            <div class="admin-form-row">
    ...
    <section class="postbox" style="margin-top:16px;">
        ...
        <div class="inside" id="regions-container">
            ...
            @foreach($zone->regions as $i => $region)
            <div class="region-row" style="display:grid; grid-template-columns:140px 1fr 40px; gap:8px; margin-bottom:8px; align-items:end;">
                <div>
                    ...
                    <select name="regions[{{ $i }}][type]">
                        <option value="pincode" @selected($region->type === 'pincode')>Pincode</option>
                        <option value="state" @selected($region->type === 'state')>State</option>
                        <option value="country" @selected($region->type === 'country')>Country</option>
                    </select>
                </div>
                <div>
                    <label style="font-size:12px; font-weight:600;">Value</label>
                    <input type="text" name="regions[{{ $i }}][value]" value="{{ $region->value }}">
                </div>
    ...
    <section class="postbox" style="margin-top:16px;">
        ...
        <div class="inside" id="methods-container">
            ...
            <div class="method-row" style="border:1px solid var(--wp-border); padding:12px; border-radius:4px; margin-bottom:12px;">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                    <div class="admin-form-row">
                        ...
                        <input type="text" name="methods[{{ $i }}][name]" value="{{ $method->name }}">
                    </div>
                    <div class="admin-form-row">
                        <label>Type</label>
                        <select name="methods[{{ $i }}][type]" onchange="toggleMethodFields(this)">
                            <option value="flat_rate" @selected($method->type === 'flat_rate')>Flat Rate</option>
                            <option value="free_shipping" @selected($method->type === 'free_shipping')>Free Shipping</option>
                            <option value="weight_based" @selected($method->type === 'weight_based')>Weight Based</option>
                        </select>
                    </div>
                </div>
                <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:8px; margin-top:8px;">
                    <div class="admin-form-row method-cost-field" @if($method->type === 'free_shipping') style="display:none;" @endif>
                        <label>Cost (₹)</label>
                        <input type="number" step="0.01" name="methods[{{ $i }}][cost]" value="{{ $method->cost }}">
                    </div>
                    <div class="admin-form-row method-free-above-field" @if($method->type !== 'free_shipping') style="display:none;" @endif>
                        <label>Free Above (₹)</label>
                        <input type="number" step="0.01" name="methods[{{ $i }}][free_above]" value="{{ $method->free_above }}">
                    </div>
                    <div class="admin-form-row method-weight-field" @if($method->type !== 'weight_based') style="display:none;" @endif>
                        <label>Min Weight (kg)</label>
                        <input type="number" step="0.01" name="methods[{{ $i }}][min_weight]" value="{{ $method->min_weight }}">
                    </div>
                    <div class="admin-form-row method-weight-field" @if($method->type !== 'weight_based') style="display:none;" @endif>
                        <label>Max Weight (kg)</label>
                        <input type="number" step="0.01" name="methods[{{ $i }}][max_weight]" value="{{ $method->max_weight }}">
                    </div>
                    <div class="admin-form-row method-weight-field" @if($method->type !== 'weight_based') style="display:none;" @endif>
                        <label>Cost/kg (₹)</label>
                        <input type="number" step="0.01" name="methods[{{ $i }}][cost_per_kg]" value="{{ $method->cost_per_kg }}">
                    </div>
                </div>
                <label class="admin-toggle-label" style="margin-top:8px;">
                    <input type="hidden" name="methods[{{ $i }}][is_active]" value="0">
                    <input type="checkbox" name="methods[{{ $i }}][is_active]" value="1" @checked($method->is_active)>
                    <span>Active</span>
                </label>
            </div>
            @endforeach
            ...
            <div class="method-row" style="border:1px solid var(--wp-border); padding:12px; border-radius:4px; margin-bottom:12px;">
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:8px;">
                    <div class="admin-form-row"><label>Method Name</label><input type="text" name="methods[0][name]" placeholder="e.g. Standard Delivery"></div>
                    <div class="admin-form-row"><label>Type</label><select name="methods[0][type]" onchange="toggleMethodFields(this)">
                ...
                <div style="display:grid; grid-template-columns:repeat(4, 1fr); gap:8px; margin-top:8px;">
                    <div class="admin-form-row method-cost-field"><label>Cost (₹)</label><input type="number" step="0.01" name="methods[0][cost]" value="0"></div>
                    <div class="admin-form-row method-free-above-field"><label>Free Above (₹)</label><input type="number" step="0.01" name="methods[0][free_above]" value=""></div>
                </div>
                <label class="admin-toggle-label" style="margin-top:8px;"><input type="hidden" name="methods[0][is_active]" value="0"><input type="checkbox" name="methods[0][is_active]" value="1" checked><span>Active</span></label>
            </div>
            @endif
    ...
    <div style="margin-top:16px; display:flex; gap:8px;">
        <button class="admin-btn" type="submit">Update Zone</button>
...
<script>
    let regionIndex = {
        {
            $zone - > regions - > count() ? : 1
        }
    };
    let methodIndex = {
        {
            $zone - > methods - > count() ? : 1
        }
    };
