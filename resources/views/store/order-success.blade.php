...
<section class="section order-success fade-in-up">
    ...
    <p class="meta" style="margin:16px auto; max-width:500px;">We've received your order and will begin processing it soon. You'll receive an email confirmation with tracking details.</p>

    <div style="display:inline-flex; gap:12px; margin-top:20px;">
        <a class="cta-btn" href="{{ route('store.account') }}">View My Orders</a>
        <a class="btn-soft" href="{{ route('store.products') }}">Continue Shopping</a>
    </div>
</section>

<section class="section fade-in-up" style="max-width:600px; margin:0 auto;">
    <div class="summary-card">
        ...
        @if($order->discount > 0)
            <div class="summary-row"><span>Discount</span><strong style="color:#00a32a;">-Rs {{ number_format($order->discount, 0) }}</strong></div>
        @endif
